...
class AdminControllerWebMvcTest {
    ...
    void getAllProjects_nonAdmin_forbidden() throws Exception {
        Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("sub", "testuser").claim("roles", List.of("USER")).build();
        mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
    ...
    void getAllProjects_emptyResponse_returnsEmptyContent() throws Exception {
        ...
        Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("sub", "adminuser").claim("roles", List.of("ADMIN")).build();
        mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
                .andExpect(status().isOk())
