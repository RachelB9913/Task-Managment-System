package com.rachel.taskManager.controller;

import com.rachel.taskManager.dto.ProjectResponseDTO;
import com.rachel.taskManager.dto.TaskResponseDTO;
import com.rachel.taskManager.service.ProjectService;
import com.rachel.taskManager.service.TaskService;
import com.rachel.taskManager.service.UserService;

import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.WebMvcTest;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.PageRequest;
import org.springframework.security.oauth2.jwt.Jwt;
import org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.data.web.config.EnableSpringDataWebSupport;

import java.util.List;

import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.when;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;


@WebMvcTest(AdminController.class)
@EnableSpringDataWebSupport
class AdminControllerWebMvcTest {

    @org.springframework.boot.test.context.TestConfiguration
    static class JwtAuthConverterTestConfig {
        @org.springframework.context.annotation.Bean
        public org.springframework.security.oauth2.server.resource.authentication.JwtAuthenticationConverter jwtAuthenticationConverter() {
            org.springframework.security.oauth2.server.resource.authentication.JwtGrantedAuthoritiesConverter grantedAuthoritiesConverter = new org.springframework.security.oauth2.server.resource.authentication.JwtGrantedAuthoritiesConverter();
            grantedAuthoritiesConverter.setAuthoritiesClaimName("cognito:groups");
            grantedAuthoritiesConverter.setAuthorityPrefix("ROLE_");
            org.springframework.security.oauth2.server.resource.authentication.JwtAuthenticationConverter jwtAuthenticationConverter = new org.springframework.security.oauth2.server.resource.authentication.JwtAuthenticationConverter();
            jwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(grantedAuthoritiesConverter);
            return jwtAuthenticationConverter;
        }
    }
    @Autowired
    private MockMvc mockMvc;
    @MockBean
    private ProjectService projectService;
    @MockBean
    private TaskService taskService;
    @MockBean
    private UserService userService;
    
    @Test
    void getAllProjects_authenticated_returnsOk() throws Exception {
        ProjectResponseDTO dto = new ProjectResponseDTO();
        dto.setId(1L);
        dto.setName("Test Project");
        dto.setDescription("Test Desc");

        Page<ProjectResponseDTO> page = new PageImpl<>(List.of(dto), PageRequest.of(0, 10), 1);
        when(projectService.getAllProjects(any())).thenReturn(page);

        Jwt jwt = Jwt.withTokenValue("token")
                .header("alg", "none")
                .claim("sub", "testuser")
                .claim("cognito:groups", List.of("ADMIN"))
                .build();
        mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].id").value(1L));
    }

    @Test
    void getAllTasks_authenticated_returnsOk() throws Exception {
        TaskResponseDTO dto = new TaskResponseDTO();
        dto.setId(1L);
        dto.setTitle("Test Task");
        dto.setDescription("Test Desc");

        Page<TaskResponseDTO> page = new PageImpl<>(List.of(dto), PageRequest.of(0, 10), 1);
        when(taskService.getAllTasks(any())).thenReturn(page);

        Jwt jwt = Jwt.withTokenValue("token")
                .header("alg", "none")
                .claim("sub", "testuser")
                .claim("cognito:groups", List.of("ADMIN"))
                .build();
        mockMvc.perform(get("/api/admin/tasks/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].id").value(1L));
    }

    // @Test
    // void getAllProjects_nonAdmin_forbidden() throws Exception {
    //     Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("sub", "testuser").claim("cognito:groups", List.of("USER")).build();
    //     // Simulate forbidden by throwing AccessDeniedException from the service
    //     org.springframework.security.access.AccessDeniedException ex = new org.springframework.security.access.AccessDeniedException("Forbidden");
    //     when(projectService.getAllProjects(any())).thenThrow(ex);
    //     mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
    //             .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
    //             .andExpect(status().isForbidden());
    // }

    // @Test
    // void getAllTasks_nonAdmin_forbidden() throws Exception {
    //     Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("sub", "testuser").claim("cognito:groups", List.of("USER")).build();
    //     // Simulate forbidden by throwing AccessDeniedException from the service
    //     org.springframework.security.access.AccessDeniedException ex = new org.springframework.security.access.AccessDeniedException("Forbidden");
    //     when(taskService.getAllTasks(any())).thenThrow(ex);
    //     mockMvc.perform(get("/api/admin/tasks/all?page=0&size=10")
    //             .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
    //             .andExpect(status().isForbidden());
    // }

    @Test
    void getAllProjects_nonAdmin_forbidden() throws Exception {
        Jwt jwt = Jwt.withTokenValue("token")
            .header("alg", "none")
            .claim("sub", "testuser")
            .claim("cognito:groups", List.of("USER")) // no ADMIN
            .build();

        mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
            .andExpect(status().isForbidden());
    }

    @Test
    void getAllTasks_nonAdmin_forbidden() throws Exception {
        Jwt jwt = Jwt.withTokenValue("token")
            .header("alg", "none")
            .claim("sub", "testuser")
            .claim("cognito:groups", List.of("USER")) // no ADMIN
            .build();

        mockMvc.perform(get("/api/admin/tasks/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
            .andExpect(status().isForbidden());
    }


    @Test
    void getAllProjects_emptyResponse_returnsEmptyContent() throws Exception {
        Page<ProjectResponseDTO> emptyPage = new PageImpl<>(List.of(), PageRequest.of(0, 10), 0);
        when(projectService.getAllProjects(any())).thenReturn(emptyPage);
        Jwt jwt = Jwt.withTokenValue("token")
                .header("alg", "none")
                .claim("sub", "adminuser")
                .claim("cognito:groups", List.of("ADMIN"))
                .build();
        mockMvc.perform(get("/api/admin/projects/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isEmpty());
    }

    @Test
    void getAllTasks_emptyResponse_returnsEmptyContent() throws Exception {
        Page<TaskResponseDTO> emptyPage = new PageImpl<>(List.of(), PageRequest.of(0, 10), 0);
        when(taskService.getAllTasks(any())).thenReturn(emptyPage);
        Jwt jwt = Jwt.withTokenValue("token")
                .header("alg", "none")
                .claim("sub", "adminuser")
                .claim("cognito:groups", List.of("ADMIN"))
                .build();
        mockMvc.perform(get("/api/admin/tasks/all?page=0&size=10")
                .with(SecurityMockMvcRequestPostProcessors.jwt().jwt(jwt)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isEmpty());
    }
}
